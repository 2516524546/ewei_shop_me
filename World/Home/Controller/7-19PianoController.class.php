...
//查看我的作业
    ...
    public function dianpin_homework(){
    	...
    	if($wid){
    		...
    		foreach ($wid as $k => $v){
    			...
    			if($a != 2){
    				$set = M("Homework_stu")->where("wid='{$v}'")->save($data);
    			}
    		...
    		if($set){
    			...
    			$hid = M("Homework_stu")->where("wid='{$wid}'")->getField('hid');
	...
	public function title_yincheng(){
		...
		if($moshi && $nandu){
			$list = M("Pblx")->field('pid as title_id,file_url')->where("lianxi='3' and moshi='{$moshi}' and nandu='{$nandu}'")->order("rand()")->limit(10)->select();
...
//首先看要不要收费，然后再看密码是不是正确 
		...
		if($a['type'] == 1 && $pwd != $a['pwd']){
			$this->error2('操作失败,密码不正确',2,'密码不正确');
		}

		if($a['pinbi'] == 1){
			$this->error2('操作失败',2,'该房间被屏蔽，不能进入');
		}
...
// 直播活动时间心跳
    ...
    public function finish_room(){
    	...
    	$finishtime = date('Y-m-d H:i:s',time()-86400); 
        $oldrooms = M('Room')->where("(activitytime <= '$oldtime' or  s_time <= '$finishtime') and 
        	                          status != '2' and kind != '2'")
                             ->select();
        //查找心跳过时的教学
        foreach ($oldrooms as $k => $v){
        	$data['e_time']  = date('Y-m-d H:i:s',time());
			$data['status'] = 2;    
   		...
   		//show房和培训课的直播室  设置关掉期限
   		// $passtime = date('Y-m-d H:i:s',time()-3600);
   		// $a = M('Room')->where("kind ='0' and activitytime > '$passtime' and
   		//                        activitytime <= '$oldtime' and s_time <= '$oldtime'")
   		//               ->setField('status',0);
   		//1小时间以内的非show房 如果被关掉了 就改回预约状态
   		
   		
     
      

		// $d = M('Room')->where("activitytime <= '$passtime'")->select();    
		// foreach($d as $k => $v){
		// 	M('Ub_guanxi')->where("zhubo_id = '$v[zhubo_id]' and room_id = '$v[room_id]'")->delete();
		// }   
  //      	M('Room')->where("status = '2'")->setField('e_time',date('Y-m-d H:i:s',time()));
...
//点击头像查看资料
	public function show_user(){
		...
		$send = M('Enjoypay')->where("user_id = '$zhubo_id'")->select();
		$a['send'] =0;
		foreach ($send as $k => $v) {
			$a['send'] += $v['money_num'];
		}

		$get = M('Enjoypay')->where("zhubo_id = '$zhubo_id'")->select();
		...
		$where = "room_id = '$room_id' and ((zhubo_id = '$zhubo_id' and user_id = '$id') or
		                                    (zhubo_id = '$id' and user_id = '$zhubo_id'))";
		
		$b = M('Ub_guanxi')->where($where)
		                   ->find();
...
//展示我的订单
    public function show_my_order(){
    	...
    	$field = 'go_name,go_num,go_money,go_userid,go_time,go_status,go_kind';
    	...
    	foreach ($a as $k => $v) {
    		...
    		if($b['kind'] == 0){
    			...
    			$a[$k]['go_price'] = $c['y_shoufei'];    
    			//总价  也就是预约时写好的价   那么当前价，就是生成订单时的价了
...
//参数,@内容、@设备号、@类型：1安卓，2苹果、@用户id
	...
	public function my_money_new(){
		$id = I('userid');
		$a = M('Users')->where("id = '$id'")->getField('money');
	...
	public function room_list(){
		...
		if($kind == 2){
			...
			foreach ($c as $k => $v){	
				...
				$b[$k]['img'] = $v['y_img'];
				$b[$k]['teacher_info'] = $v['y_teacher_info'];
...
//获取唱谱文件
	...
	// 	$a = M('Room')->where("room_id = '$room_id'")->find();
	// 	if($a['kind'] == 2){
	// 		$b = M('Goods_order')->where("go_name like '%$room_id%' and go_type = '1' and 
	// 			                          go_userid = '$id'")
	// 		                     ->find();
	// 		$c = M('Yuyue')->where("y_roomid = '$room_id'")->getField('y_part');
	// 		$d = M('Yuyue')->where("y_part = '$c'")->select();
...
//提交订单
    public function add_goods(){
    	...
    	$data['go_price	'] = I('price');
    	$data['go_kind'] = 1;
    	if(empty($data['go_money'])){
    		$data['go_money'] = 0;
    		$data['go_kind'] = 2;
    	}
    	$data['go_point'] = I('point');
...
//获取可拉入班级的人员
	public function  get_person(){
		...
		foreach ($b as $k => $v){
			...
			if(stripos($idss,$v) !== false){//为什么这里的stripos  又可以呢？
				unset($c[$k]);
			}//其他班的学生
...
//查找-搜索用户
	public function search_user(){
		...
		$stu = trim($stu,',');//‘我’的学生 去掉重复的  通过转换数组，然后去掉数组里相同的值，再换回字符串
		$stu = array_unique(explode(',',$stu));
		$stu = implode(',',$stu);

		$b = M('User_friends')->where("userid = '$id' and u_status = '1' and 
			                           friend_id like '%$phone%' and role != '2'")
		                      ->join('piano_users on piano_users.id = piano_user_friends.friend_id')
		                      ->field('id as userid,nickname,headerimg,role')
		                      ->limit(($page-1)*10,10)
		                      ->select();

		foreach ($b as $k => $v){
			if(stripos($stu,$v['userid']) !== false){
				$b[$k]['id_student'] = 1;  //表示是‘我’的学生
			}else{
				$b[$k]['id_student'] = 2;  //不是
			}
		}
...
//添加作业  5-20 新版需求
	public function homework_add1(){
		...
		foreach ($stu_arr as $k => $v) {
			$tea = M('Teacher')->where("t_userid = '$v'")->find();
			if($tea){
