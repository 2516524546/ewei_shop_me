...
class HxController extends Controller{
	 ...
	 * 环信-服务器端REST API
	 */
	private $client_id = 'YXA6FPo7wAZkEei5yPdzDi1FaA';
	private $client_secret = 'YXA6eg7OLC53BHn63w1byI-l1ju5ky8';
	private $org_name = '1167180131178916';
	private $app_name = 'klqhapp';
	private $url = 'https://a1.easemob.com/1167180131178916/klqhapp/';
	/**
	 * 开放注册模式
	 *
	...
	// 聊天室
	public function createChat($option)
	{
	...
	// 删除聊天室
	public function removeChat($group_id) {
		$url = $this->url . "chatrooms/" . $group_id;
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
		$result = $this->postCurl ( $url, '', $header, $type = "DELETE" );
		return $result;
	...
	// 聊天室踢出用户
	public function removeUser($chatroomid,$username) {
	...
	// 添加用户到聊天室
	public function AddChat($chatroomid,$username)
	{
		$url = $this->url . "chatrooms/" . $chatroomid . "/users/" . $username;
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
	...
	public function chatGroupsDetails($group_id) {
		...
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
		$result = $this->postCurl ( $url, '', $header, $type = "
		" );
		return $result;
	}
	/**
	...
	public function hx_send($sender, $receiver, $msg)
	{
		...
		return $this->postCurl($url, $data, $header, "POST");
	}
