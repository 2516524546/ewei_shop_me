...
class PianoController extends CommonController {
	public function jiguang11(){
        ...
        $stu_loing = M("Token")->where("t_token = '$token'")->find();
       $payload = $client->push()
										    ->setPlatform('ios')
										    ->addAlias($stu_loing['t_shebei'])
							  ...
							  'extras' => [
							    'key' => 'value'
										    ...
										    ->build();
$response = $client->schedule()->createSingleSchedule("指定时间点的定时任务", $payload, array("time"=>date("Y-m-d H:i:s",
								 												 $time1)));
...
//     ->setPlatform('ios')
//     ->addAlias($stu_loing['t_shebei'])
//     ->setNotificationAlert("Hi, 123")
//     ->build();
// $response = $client->schedule()->createSingleSchedule("每天14点发送的定时任务", $payload, array("time"=>date("Y-m-d H:i:s",
// 								 												 $time1)));
	 //   $payload = $client->push()
  //     ->setPlatform('ios')
  //     ->addAlias($stu_loing['t_shebei'])
  ...
  // //创建在指定时间点触发的定时任务
  // $response = $client->schedule()->createSingleSchedule("指定时间点的定时任务",$payload, array("time"=>$time1));
  if($response){
  	$rest['msg'] = '成功';
	$rest['status'] = true;
  ...
  //echo 'Result=' . json_encode($response); 				                                           	      
	...
	public function course_score(){
		...
		if($list['cl_etime'] > time()){
			$this->error2('评分失败,课程尚未开课',2,'课程尚未开课');
		}
   ...
   //课程详情
    public function course_xiang(){
	   	...
	   	if($kid){
	   		...
	   		$field = "piano_users.username,piano_users.nickname,piano_course.course_address,piano_course.course_number,piano_course_list.*,piano_users.id,piano_users.headerimg";
	   		...
	   		$turename = M('Teacher')->where("t_userid = '$course[cl_tid]'")->getField('t_truename');
	   		...
	   		$data = array(
	   				...
	   				'headerimg'=>$course['headerimg'],
	   				'userid'=>$course['id'],
	   				'truename'=>$turename?$turename:''
	   		);
...
//课程留言    
   ...
   public function memo_add(){
        ...
        if(!$this->check_token($userid, $token)){
       	...
       	if($stime < time() + 1800 ){
			$this->error2("只能添加半个小时候以后的备忘录",2,'');
		}
		
   	    $shebei = M("Token")->where("t_userid='{$userid}'")->getField("t_shebei");
   	    $leixin = M("Token")->where("t_userid='{$userid}'")->getField("t_leixin");
   	    ...
   	    if($title && $content && $stime && $etime){
            ...
            if($add){
            			if($leixin == 1){
						    ...
						    ->setPlatform('android')	               
		                    ->addAlias($shebei)
						    ->setNotificationAlert("您的备忘录有一条记录，离开始时间还剩30分钟")
						    ->build();
							$response = $client->schedule()->createSingleSchedule("指定时间点的定时任务",
	      					$payload, array("time"=>date("Y-m-d H:i:s", $stime-1800)));
            			}else if($leixin == 2){
             				$payload = $client->push()
										    ->setPlatform('ios')
										    ->addAlias($shebei)
										    ->iosNotification('您的备忘录有一条记录，离开始时间还剩30分钟', [
							  'sound' => 'sound.caf',
							  'badge' => '1',
							  'extras' => [
							    'key' => 'value'
...
//添加备忘录
    public function memo_add1(){
        ...
        if($stime < time()){
         	$this->error2('时间不能小于当前时间',2,'时间不能小于当前时间');
        }
            	...
            	// if(time() <= $stime - 1800){
            		if($leixin == 1){
						$payload = $client->push()
					                      ->setPlatform('android')	               
	                                      ->addAlias($shebei)
					                      ->setNotificationAlert("您的备忘录有一条记录")
					                      ->build();
						$response = $client->schedule() 
						                   ->createSingleSchedule("指定时间点的定时任务",
	  					                                           $payload, 
...
// 								 												 $stime)));
      ...
      //           ))
    			...
    			// }else{
    			// 	$time = (time()-$time)/60;  //30分钟内的推送
    			// 	if($leixin == 1){
    			// 		$response = $client->push()
       //                                     ->setPlatform('android')
       //                                     ->addAlias($shebei)
       //                                     ->androidNotification("您的备忘录有一条记录，离开始时间还剩".$time."分钟", array('title' => '备忘录消息提醒','extras' => array(
							// 				                  'key' => 'value','jiguang'),))
       //                                     ->send();
    			// 	}else{
    			// 		$response = $client->push()
       //                                     ->setPlatform('ios')
       //                                     ->addAlias($shebei)
       //                                     ->iosNotification("您的备忘录有一条记录，离开始时间还剩".$time.
       //                                     	                 "分钟", array('sound' => 'sound.caf',
       //                                      	              'badge' => '+1','category' => 'jiguang',
       //                                      	              'key' => 'value','jiguang'))
       //                                     ->send();
    			// 	}	
    			// }
	...
	public function show_homework(){
		...
		$c = M('Homework')->where("hid in ($ids)")
		                  ...
		                  ->field('hid,piano_homework.lid,list_name,list_url,list_url_music,speed,
		                  	       h_status as status,sid,point,count,stime,etime,type')
		                  ->limit(($page-1)*10,10)
		                  ->order('hid DESC')
		                  ->select();   
		foreach ($c as $k => $v){
...
//查看我的作业
		...
		if($user == 1){
			if(empty($wid) && empty($stu_id)){
				...
				foreach ($a as $k => $v) {
					...
					// $b[$k]['total_time'] = M('Homework_stu')->where("hid = '$v[hid]' and w_userid = '$id'")
					//                                         ->sum('w_long');
				}
			}
			...
			if($wid && $stu_id){
				$a = M('Homework_stu')->where("w_userid = '$stu_id' and hid = '$wid'")
				                      ->select();
				$hid = $a[0]['hid'];
				$homework = M('homework') -> where("hid ='$hid'") ->find();
				...
				$b['speed'] = $a[0]['w_speed'];
				$b['s_point'] = M('Homework')->where("hid = '$hid'")->getField('point');
			...
			if($wid && $stu_id){
				...
				$b['name'] = M('Songs_list')->where("lid = '$lid'")->getField('list_name');
				$b['status'] = $a[0]['h_status'];  
				$b['speed'] = $a[0]['w_speed'];
				$b['s_point'] = M('Homework')->where("hid = '$wid'")->getField('point');
	...
	public function course_history(){
		...
		foreach($list as $k=>$val){
			...
			$img = M("Course_file")->field('img_url')->where("kid='{$val['id']}'")->find()['img_url'];
			//课程作业
	...
	public function upload_course_file1(){
		$userid = (int)$_REQUEST['userid'];
     	$token = trim($_REQUEST['token']);
     	if(!$this->check_token($userid, $token)){
     		$this->error2('添加失败',1,'尚未登录');
		...
		if($file1){
			...
			$info1   =   $upload->uploadOne($file1);
			if(!$info1) {// 上传错误提示错误信息
				$this->error2('上传图片失败',2,$this->error($upload->getError()));
			}else{// 上传成功 获取上传文件信息
				$img_name= $upload->rootPath.$info1['savepath'].$info1['savename'];
			}
		}
		if($file){
			$upload = new \Think\Upload();// 实例化上传类
			...
			$info   =   $upload->uploadOne($file);
			if(!$info) {// 上传错误提示错误信息
				$this->error2('上传视频失败',2,$this->error($upload->getError()));
			}else{// 上传成功 获取上传文件信息
				$file_name= $upload->rootPath.$info['savepath'].$info['savename'];
			}
		}
	...
	public function upload_course_file(){
		...
		// 			  'img_url'  =>$img_url
		// 			     );
		// 		$set = M('Course_file')->where("kid='{$kid}'")->save($data);
		...
		// 		$rest['status'] = true;
		// 		$rest['msg'] = '上传成功';
		// 		$rest['filename'] = $file_url;
		// 		$rest['imgname'] = $img_url;
		// 		echo $this->json_encode_ex($rest);exit;
		// 	}else{
		// 		$this->error2('上传失败',1,'sql错误');exit;
		// 	}
		// }else{
		// 	$this->error2('上传失败',2,'未选择文件');exit;
		// }
	...
	public function course_edit_time(){
		...
		if($course){
			...
			$this->error2('操作失败',1,'开课前24小时内无法修改时间');
		}
	...
	public function course_teacher(){
		...
		if($userid && $type){
			...
			if($user){
				...
				$course = M("Course_list")->field($field)->order('cl_stime asc')->join($join)->where($where)->select();
	 ...
	 //    			exit;
	    ...
	    if($user){
	    	if($address && $time && $long && $type && $number && $stuid){
	    		...
	    		//echo M("Course_list")->getLastSql();exit;
	    		if($teac){
	    			$this->error2('当前时段已有课,请更换时间~',2,'当前时段已有课,请更换时间~');
	    		}
	    		//学生相同时段不能添加课程
	    		$stu_arr = explode(',',$stuid);
	    		foreach($stu_arr as $val){
	    			...
	    			if($stu){
	    				...
	    				$this->error2('添加失败,同学在当前时间段已经有课了,请更换时间~',2,$nickname.'同学在当前时间段已经有课了,请更换时间~');
	    			}
	    		}
	    			...
	    			$shebei = M("Token")->where("t_userid='{$userid}'")->getField("t_shebei");
   	    			...
   	    			if($leixin == 1){
						$payload = $client->push()
					                      ->setPlatform('android')	               
	                                      ->addAlias($shebei)
					                      ->setNotificationAlert("您有一节课即将开课，记得去上哦~")
					                      ->build();
						$response = $client->schedule() 
						                   ->createSingleSchedule("指定时间点的定时任务",
	  					                                           $payload, 
	    	 			 ...
	    	 			 $payload = $client->push()
										    ->setPlatform('ios')
										    ->addAlias($shebei)
										    ->iosNotification('您有一节课即将开课，记得去上哦~', [
							  'sound' => 'sound.caf',
							  'badge' => '1',
							  'extras' => [
							    'key' => 'value'
...
// 								 												 $time)));
	    	// 			 $client->push()
						//     ->setPlatform('ios')
						//     ->addAlias($stu_loing['t_shebei'])
						//     ->setNotificationAlert('老师给你添加了一个课程,请留意,记得准时去上~')
						//     ->send();
	    	// // 			$payload = $client->push()
					 // //                      ->setPlatform('ios')	               
	     // //                                  ->addAlias($shebei)
	  			...
	  			// // 		                                           	      $time)));
	    				...
	    				if($stu_loing){
	    					if($stu_loing['t_leixin'] == 2){
	    						$response = $client->push()
												    ->setPlatform('ios')
												    ->addAlias($stu_loing['t_shebei'])
												    ->iosNotification('老师给你添加了一个课程,请留意,记得准时去上~', [
																	  'sound' => 'sound.caf',
																	  'badge' => '1',
																	  'extras' => [
																	    'key' => 'value'
	    			...
	    			// 			$payload = $client->push()
								...
								// 				  ->setPlatform('ios')	               
								// 				 ->addAlias($stu_loing['t_shebei'])
								// 				 ->setNotificationAlert("老师给你添加了一个课程,请留意,记得准时去上~", array(
								// 		              'sound' => 'sound.caf'
					   //                              ))
								// 				  ->options(array(
        // 											  "apns_production" => true  
     			// 									))
	    			...
	    			$this->error2('添加失败,sql错误',1,'sql错误');
	    		}	
	    	}else{
	    		$this->error1();
	    	}
	    }else{
	    	$this->error2('添加失败,非法操作',1,'非法操作');
	    }
	}
	//匹配手机通讯录返回注册过的用户
	...
	public function friends_add(){
		...
		$list = M("User_friends")->where("(userid='{$userid}' and friend_id='{$friend_id}') or (userid='{$friend_id}' and friend_id='{$userid}') and u_status='1'")->select();
		if($list){
			$this->error2('你们已经是好友了',1,'重复添加');
		}
        if($userid != $friend_id){
  ...
  return $array;
	...
	public function xiaoxi_list(){
		...
		foreach($list as $k=>$v){
			...
			$data[$k] = array(
							...
							'time' => $v['x_time'],
							'status' => $v['x_status'],
					        'img' => $header?$header:'./Public/%7BA6373E3C-5545-76C0-946E-593ED5944DB9%7D.png',
					        'type' => $v['x_type'],
					        'x_kid'=>$v['x_kid'],
			...
			$arr = $this->object2array($data[$k]['x_info']);
			$time = time();
	...
	public function title_shipu(){
		...
		if($pubiao  && $yinfu){
			$list = M("Pblx")->field('pid as title_id,file_url')->where("lianxi='1' and pubiao='{$pubiao}' and yinfu='{$yinfu}'")->order("rand()")->limit($pages)->select();
			...
			if($list){
				...
				$this->error2('暂无可匹配数据，换其它类型题目试试',2,'亲，暂时没有数据噢');
			}
		}else{
			$this->error1();
		}
	...
	public function title_yinjie(){
		...
		if($pubiao && $yinfu){
			$list = M("Pblx")->field('pid as title_id,file_url')->where("lianxi='2' and pubiao='{$pubiao}' and yinfu='{$yinfu}'")->order("rand()")->limit(10)->select();
	...
	public function title_tingyin(){
		...
		if($yinyu && $fuyin){
			$list = M("Pblx")->field('pid as title_id,file_url')->where("lianxi='4' and yinyu='{$yinyu}' and yinfu='{$fuyin}'")->order("rand()")->limit($pages)->select();
			// dump($list);
			// die;		
			foreach ($list as $k => $v) {
				if(!$v['title_id']){
...
//基础练习成绩提交
	public function add_pblx(){
		// $pid = I('title_id')?I('title_id'):$this->error1();
		// dump(I('right_num'));
		// die;
		$userid = I('userid')?I('userid'):$this->error1();
		if(I('right_num') == '0'){
			$rit = 0;
		}else{
			$rit = I('right_num')?I('right_num'):$this->error1();
		}
		if(I('wrong_num') == '0'){
			$wrt = 0;
		}else{
			$wrt = I('wrong_num')?I('wrong_num'):$this->error1();
		}
		

		$data = array('p_userid'=>$userid,'p_right'=>$rit,'p_wrong'=>$wrt,
			          'p_time'=>date('Y-m-d H:i:s',time()));

		$b = M('Pblx_log')->where("p_userid = '$userid'")->order('p_time DESC')->find();
		$time = strtotime($b['p_time']);

		if($time < time()-30){ //防重复提交
			$a = M('Pblx_log')->add($data);
		}else{
			$this->error2('提交失败,提交不要过于频繁',2,'');
		}
	...
	public function index_sousou(){
	    ...
	    if($type && $key){
	    	if($type == 1){
	    		...
	    		// $a = M('zhubo_reg') ->alias('zb')
		//                   ->join('__DEALERS__ d on d.jxid = zb.r_zbid') 
		// 				  ->join('__ROOM__ r on zb.zb_id = r.zhubo_id')
		// 				  ->where("zb.r_zbid = '{$id}' and r.pinbi = '0' and r.status = '1' and zb.zb_kind = '1'")
		// 				  ->limit(($page-1)*10,10)
		// 		          ->select();
	    		$a = M('Luntan')->where("l_type ='3' and l_pinbi='1' and 
	    						...
	    						->field('piano_luntan.tiezi_id,piano_luntan.l_title,piano_users.headerimg,
	    							     piano_users.nickname,piano_luntan.l_addtime,l_history_num,
	    							     piano_luntan.l_img,piano_luntan.l_content,piano_luntan.l_type')
	    						->limit(($page-1)*10,10)
	    					    ->select();	
	    					    // dump($a);
	    					    // die;
	...
	public function my_data(){
    	...
    	if($b){
    		...
    		M('Goods_order')->where("go_userid = '$user[id]' and go_status = '3'")->setField('go_status',1);
    	}
	...
	public function login(){
		...
		require_once('./Piano/Common/Lib/Easemob.class.php');
		$array = array(
				'client_id' => 'YXA6FPo7wAZkEei5yPdzDi1FaA',
				'client_secret' => 'YXA6eg7OLC53BHn63w1byI-l1ju5ky8',
				'org_name' => '1167180131178916',
				'app_name' => 'klqhapp'
			);
		...
		if($phone && $pwd != '' && $shebei && $leixin){
			...
			if($a){
				...
				$userInfo = $easemob->getUser($hx_id);
			
				if(empty($userInfo['entities'])){
					
					$options['username'] = $hx_id;
					$options['password'] = $hx_id;
					$Hx->createUser($options);
				   ...
				   echo json_encode($rest, JSON_UNESCAPED_UNICODE);
					if($login['t_shebei'] == $shebei){
					   		...
					   		if($login['t_leixin'] == 2 && $login['t_shebei']){
					   			$this->ios_msg_send($content,$login['t_shebei']);
					   		}elseif($login['t_leixin'] == 1 && $login['t_shebei']){
	...
	public function register(){
		...
		if($phone && $nickname && $password){
			...
			$data = array(
				'username' => $phone,
				'pwd' => md5(MA.$password),
				'headerimg' => $headerimg?$headerimg:'./Public/%7BA6373E3C-5545-76C0-946E-593ED5944DB9%7D.png',
				'nickname' => $nickname,
				'sex' => $sex,
				'parent_id' => $parent_id,
				// 'yaoqingma' =>$yaoqingma,
			...
			//注册环信
			$Hx = new HxController();
			$userInfo = $Hx->userDetails($add);
			$ress = json_decode($userInfo,true);
			if(empty($ress['entities'])){
...
// 用户身份role=2
	...
	public function show_daili(){
		...
		// die;
			$jxid = $a['t_jxid'];
			$b=M('Dealers')->where("jx_userid = '$id'")
...
//认证琴行直播
	public function daili_renzhen(){
		...
		$zhiboid = $a['jx_zhiboid'];
		if($zhiboid == 3){
			$this->error2('该经销商已经认证通过不能重新认证',2,'该经销商已经认证通过不能重新认证');
		}
		...
		$d = M('Teacher')->where("t_userid = '$data[jx_userid]'")->setField('t_jx','2');
...
//主播认证     5-20新需求
	public function zhubo_renzhen(){
		...
		if($data['zb_kind'] == 1){
			...
			if($b){
				...
				if($d['jx_zhiboid'] != '3'){
					// $rest['status'] = false;
					// $rest['msg'] = '所属琴行直播认证尚未获取';
					// echo json_encode($rest, JSON_UNESCAPED_UNICODE);
					$this->error2('所属琴行直播认证尚未获取',2,'所属琴行直播认证尚未获取');
	...
	public function new_room(){
		...
		$arr1 = substr($str,-9); 
		$arr = '1'.$arr1;
		if(M('Room')->where("room_id = '$arr'")->find()){
			$arr = $this->new_room();
		}
		// echo $arr;exit;
		return $arr;
...
//观众进直播室
	public function add_guanzhong(){
		...
		//首先看要不要收费，然后再看密码是不是正确 
 		...
 		}elseif($find['kind'] == '0' && $find['live_shoufei'] != '0'){//show房  收乐币

 			$buy = M('Show_buy')->where("show_userid = '$user_id' and show_room_id = '$room_id'")->find();
 			if(!$buy){
 				$money = M('Users')->where("id = '$user_id'")->getField('money');
				if(bccomp($find['live_shoufei'],$money,1) == '1'){
					$this->error2("亲，您的乐币不够，快去充值吧",6,"乐币不够");
				}else{
					//买票 扣钱 记录收入  主播加钱
					$data['show_userid'] = $user_id;
			    	$data['show_room_id'] = $room_id;
			    	$data['show_price'] = $find['live_shoufei'];    //乐币数
			    	$data['show_time'] = date('Y-m-d H:i:s',time());
			    	M('Show_buy')->add($data);

			    	$new_money = bcsub($money,$find['live_shoufei'],1);
			    	M('Users')->where("id = '$user_id'")->setField('money',$new_money);

			    	$shouru = M('Room')->where("room_id = '$room_id'")->getField('total_money');
			    	$new_shouru = bcadd($shuru,$find['live_shoufei'],1);
			    	M('Room')->where("room_id = '$room_id'")->setField('total_money',$new_shouru);

			    	$t_money = M('Users')->where("id = '$find[zhubo_id]'")->getField('money');
			    	$add_money = bcadd($t_money,$find['live_shoufei'],1);
			    	M('Users')->where("id = '$find[zhubo_id]'")->setField('money',$add_money);
			    }
 			}
 		}
		...
		if($a['pinbi'] == 1){
			$this->error2('该房间被屏蔽，不能进入',4,'该房间被屏蔽，不能进入');
		}
		...
		//查找主播与用户的关系
		if($b['ti'] == 1){
			$this->error2('被踢，不能进',5,'被踢，不能进');	
		}

		if($find['kind'] == 1){
			if(!$find['role_name']){
				M('Room')->where("room_id = '$room_id' and pinbi = '0'")->setField('role_name',$user_id);
			}else{
				if($find['role_name'] != $user_id){
					$this->error2('一对一私教房间，您无权限',7,'一对一私教房间，您无权限');
				}
			}
		}
	...
	public function find_room_money(){
		$room_id = I('room_id');
		$a = M('Room')->where("room_id = '$room_id'")->getField('total_money');
...
//刷新在场观众
	public function show_guanzhong(){
		...
		$a = M('Ub_guanxi')->order('gx_id DESC')
						   ...
						   ->field('piano_users.headerimg as photo,piano_users.nickname,piano_users.id as userid')	
						   ->select();
...
// 直播活动时间心跳
    ...
    public function finish_room(){
    	$oldtime = date('Y-m-d H:i:s',time()-120); //两分钟之前的时间
    	$finishtime = date('Y-m-d H:i:s',time()-3600); //一个小时之前
    	//                      心跳时间                     正在或预约直播     直播不是培训课程
        $a = M('Room')->where("activitytime <= '$oldtime' and status != '2' and kind != '2' and 
        	                   s_time <= '$finishtime'") //直播开始时间在一小时之前
                      ->select();
        //show房关掉,教学关掉
        foreach ($a as $k => $v){
        	$data['e_time']  = date('Y-m-d H:i:s',time());
...
//帖子列表
	...
	//查询某个帖子详情
	public function tiezi_one(){
		$id = I('userid');
		$tieziid = I('tiezi_id');
		$a = M('luntan') -> where("tiezi_id = '$tieziid'") -> find();
		$b['tiezi_id'] = $a['tiezi_id'];
		$b['l_history_num'] = $a['l_history_num'];
		//$b['headerimg'] = 
		$b['l_addtime'] = $a['l_addtime'];
		
			if($a['l_img']){
				$b['l_img'] = explode(',',$a['l_img']);
			}else{
				$b['l_img'] = array();
			}
		...
		$b['l_video'] = $a['l_video'];
		$b['l_photo'] = $a['l_photo'];
		$c =  M('Focus')->where("f_userid = '$a[l_userid]' and f_fansid = '$id'")
                               ->find();
       if($c){
              if($c['f_status'] == 1)
              {
        ...
        $b['l_userid'] = $a['l_userid'];
        $b['l_content'] = $a['l_content'];
        $b['zan_num'] =  M('Ltguanxi')->where("tz_id = '$a[tiezi_id]' and lg_zan = '2'")->count(); 
        $b['shoucang_num'] = M('Ltguanxi')->where("tz_id = '$a[tiezi_id]' and lg_shoucang = '2'")->count();
        $b['pinlun'] = M('Reply')->where("retz_id = '$a[tiezi_id]'")->count();
        $b['l_jinhua'] = $a['l_jinhua'];
        $d = M('users') -> where("id = '$a[l_userid]'")-> find();
        $b['headerimg'] = $d['headerimg'];
	...
	public function tiezi_xiang(){
		...
		$a['pinlun'] = M('reply')-> where("retz_id = '$id' and re_fen ='1'" )->count();
...
//充值
	public function chongzhi(){
		// $a = $this->careat_Wxpay_sn('H763296266aa43050687397', '10.00', '支付');
		// dump($a);exit;
	...
	/*	if($k != 0){
		...
		if($b){
			if($data['xf_kind'] ==2){
				$result['Wxpay'] = $this->careat_Wxpay_sn($data['xf_num'], $data['xf_money'], '支付');
			}
	...
	//充值回调
	public function cz_huidiao(){
		$num = I('num');
		$id = I('userid');
		$user = M('Users')->where("id = '$id'")->find();
		if($user){
			$money = $user['money'];
			$list = M('Xnb_record') -> where("xf_num = '$num'")->find();
			if($list){
				$point = $list['xf_point'];
				$money1 = $money+$point;
				$save = M('Xnb_record') -> where("xf_num = '$num'") -> setField('xf_status',2);
				$save1 = M('Users') -> where("id = '$id'") -> setField('money',$money1);
				$result['status']=true;
	            $result['message']='充值成功';
	            echo json_encode($result,JSON_UNESCAPED_UNICODE);exit;
			}else{
...
//生成微信预付单号
    public function careat_Wxpay_sn($order_sn,$price,$type){
        ...
        $input->SetBody("乐乐GO".$type);
        $input->SetAttach("乐乐GO");
        $input->SetOut_trade_no($order_sn);
        $input->SetTotal_fee($price);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("乐乐GO");
...
//申请提现接口
    public function tixian(){
        ...
        $data['xf_money']= bcdiv($data['xf_point'],$ss['c_point'],3);  //可提现出来的现金数额
        $data['xf_money']= sprintf("%.2f",$data['xf_money']);
...
//Top10系列   
    public function hot_tiezi(){
    	...
    	$a = M('Luntan')->order('l_history_num DESC , l_addtime DESC')
    	                ...
    	                ->join($join)
    	                ->field('id,headerimg,nickname,tiezi_id,l_addtime,l_title,l_history_num,lv as level')
    	                ->limit(10)
    	                ->select();
    ...
    public function hot_huodong(){
		...
		$a = M('Luntan')->order('l_history_num DESC , l_addtime DESC')
    	                ...
    	                ->join($join)
    	                ->field('id,headerimg,nickname,tiezi_id,l_img,l_title,l_addtime,l_history_num')
    	                ->limit(10)
    	                ->select();
    	foreach($a as $k => $v){
    		if($v['l_img']){
	...
	//    $hao = I('sb_hao');
  	...
  	public function jiubao(){
  		...
  		if($a){
  			...
  			$data['jib_time'] = date('Y-m-d H:i:s',time());
...
//风云榜  财富top10
  	public function money_top10(){
  		$a = M('Users')->order('money DESC')
  		               ->limit(10)
  		               ->field('id as userid,nickname,sex,headerimg,sign,money,lv as level')
  		               ->select();
  		foreach ($a as $k => $v) {
  			if(empty($v['nickname'])){
				$a[$k]['nickname'] = '';
...
//勤奋榜
  	public function work_top10(){
  		$join = 'piano_users on piano_users.id = piano_practice_log.log_userid';
  		$a = M('Practice_log')->field('sum(log_long) as total,log_userid as id,lv')
  							  ->group('log_userid')
  							  ->join($join)
...
//满分榜
  	public function full_work_top10(){
  		$join = 'piano_users on piano_users.id = piano_practice_log.log_userid';
        $a = M('Practice_log')->field('count(*) as count,log_userid as id,lv')
                              ->where("log_point = '100'")
	                          ->group('log_userid')
	                          ->join($join)
  	...
  	public function dianpin_top10(){
  		$a = M('Advice')->group('ad_user')
  		                ...
  		                ->field('id as userid,nickname,sex,headerimg,count(ad_user) as count,lv as level')
  		                ->limit(10)
  		                ->select();
  	...
  	public function luntan_top10(){
  		$a = M('Luntan')->where("l_jinhua = '2'")
  						...
  						->field('id as userid,nickname,sex,headerimg,count(l_jinhua) as count,lv as level')
  						->limit(10)
  						->select();
  		foreach ($a as $k => $v) {
  			if(empty($v['nickname'])){
...
//参数,@内容、@设备号、@类型：1安卓，2苹果、@用户id
	...
	public function my_money_new(){
		...
		if($b){
			...
			M('Goods_order')->where("go_userid = '$id' and go_status = '3'")->setField('go_status',1);
		}
	...
	public function luo(){
		...
		$field = "piano_video.vid,piano_video.v_type,piano_video.v_fileurl,piano_songs_list.list_name as lname,piano_songs.songs_name as sname";
			$join1 = "piano_songs_list on piano_songs_list.lid = piano_video.v_lid";
			$join2 = "piano_songs on piano_songs_list.list_sid = piano_songs.sid";
			$where = "piano_video.v_fileurl='{$url}' ";
...
//预约直播   5-20新需求
	public function yuyue_peixun(){
		...
		for($i=1;$i<=$count-1;$i++){
			...
			$data1['y_time'] = date('Y-m-d H:i:00',strtotime($list[$x]['time']));
	...
	public function yuyue(){
		...
		$data['y_time'] = date('Y-m-d H:i:00',strtotime(I('time')));
		$data['kind'] = 0;
		$data['y_part'] = 0;
...
//发布直播  5-20新需求
	public function start_live(){
		...
		$x = M('Room')->where("zhubo_id = '$id' and status = '1' and kind = '1'")->find();
		if($x){
			M('Room')->where("r_id = '$x[r_id]'")->setField('status',2);
		}//允许主播掉线后马上重新开直播

		$y = M('Room')->where("room_id = '$room_id' and status = '1' and kind = '0'")->find();
		if($y['s_time'] < date('Y-m-d H:i:s',time()-600)){
			M('Room')->where("r_id = '$y[r_id]'")->setField('status',2);
		}else{
			M('Room')->where("r_id = '$y[r_id]'")->setField('status',0);
		}//开播时间10分钟之内，允许主播掉线后马上重新开直播，过后再想重播就直接关掉了

		$z = M('Room')->where("room_id = '$room_id' and status = '1' and kind = '2'")->find();
		if(!$z['url'] || empty($z['url'])){
			M('Room')->where("zhubo_id = '$id' and status = '1' and kind = '0'")->setField('status',0);
		}//培训则是一定要上传视频才能关掉的
		...
		if($kind != 1){
			$l = M('Room')->where("room_id = '$room_id'")->getField('status');
			if($l == 2){
				$this->error2('发起直播失败,您已发布过该直播',2,'直播已结束');
			}
	...
	public function peixun_detail(){
    	$yid = I('kid'); //主课id
    	$userid = I('userid'); //用户id
    	$list = M('yuyue') -> where("yid = '$yid'") -> find(); //主课信息
    	// dump($list);
    	// die;
    	
    	if($list){
    		$b['zhubo_id'] = $list['zhubo_id'];
    		$b['content'] = $list['y_content'];
    		$b['teacher_info'] = $list['y_teacher_info'];
    		$b['headerimg'] = M('Users')->where("id = '$list[zhubo_id]'")->getField('headerimg');
    		$b['sign'] = M('Users')->where("id = '$list[zhubo_id]'")->getField('sign');
    		$b['nickname'] = M('Users')->where("id = '$list[zhubo_id]'")->getField('nickname');
    		$b['title'] =  $list['y_title_name'];
    		// dump($b);
    		$a = M('yuyue') -> where("y_part = '$list[yid]'") -> select();
    		$time = time();

    		if($a){
    			foreach ($a as $k => $v) {
    				$b['trainLists'][$k]['teacher_info'] = $v['y_teacher_info'];
    				$room_id = $v['y_roomid'];
    				$room = M('room') -> where("room_id = '$room_id'") -> find();
    				
    				$b['trainLists'][$k]['s_time'] = $room['s_time'];

    				if($room['status'] == 0){
    					$b['trainLists'][$k]['status'] = '0';
    				}
    				if($room['status'] == 1){
    					$b['trainLists'][$k]['status'] = '1';
    				}
    				if($room['status'] == 2){
    					$b['trainLists'][$k]['status'] = '2';
    				}
    				$b['trainLists'][$k]['room_id'] = $room['room_id'];
    				$b['trainLists'][$k]['live_shoufei'] = $room['total_money'];
    				$b['trainLists'][$k]['title_name'] = $v['y_title_name'];
    				$b['trainLists'][$k]['zhubo_name'] = $room['zhubo_name'];
    				$b['trainLists'][$k]['zhubo_id'] = $room['zhubo_id'];
    	// 			$b = $this->PlayInfoimg($v['url']);//通过这个接口去查地址
					// $c = $b['fileSet'][0]['playSet'][0];
					// $a[$k]['url'] = $c['url'];
					$f = $this->PlayInfoimg($room['url']);//通过这个接口去查地址
					$g = $f['fileSet'][0]['playSet'][0];
    				$b['trainLists'][$k]['url'] = $g['url'];
					if($v['y_status'] != 2){
						$b['trainLists'][$k]['price'] = $v['y_shoufei'];
					}else{
						$b['trainLists'][$k]['price'] = $v['y_video_shoufei'];
					}

    				if($userid){
						$where = "go_userid = '$userid' and go_type = '1' and 
					          	  go_name like '%$room[room_id]%'";
					...
					if($buy){
						if($buy['go_status'] == 2){
							$b['trainLists'][$k]['buy'] = '2';
						}else{
							$b['trainLists'][$k]['buy'] = '1';
						}
					}else{
						$b['trainLists'][$k]['buy'] = '0';
					}	
    			}
    		}
    	}
    ...
    public function check_buy(){
    	$yid = I('sub_id'); //子课id
    	$userid = I('userid'); //用户id
    	$yuyue = M("yuyue") -> where("y_roomid = '$yid'") -> find();
    	 // dump($yuyue);
    	 // die;
    	$b['kid'] = $yuyue['y_part'];
    	$video_shoufei = $yuyue['y_video_shoufei'];
    		
    	if($userid){
						$where = "go_userid = '$userid' and go_type = '1' and 
					          	  go_name like '%$yuyue[y_roomid]%'";
	...
	public function room_list(){
		...
		if($kind == 0){
			...
			foreach($a as $k=>$v){
				...
				$b[$k]['nickname'] = M('Users')->where("id = '$v[zhubo_id]'")->getField('nickname');
				...
				$b[$k]['kind'] = 0;
				if($id){
					$where = "show_userid = '$id' and show_room_id = '$v[room_id]'";
				          
					if($id != $v['zhubo_id']){
						$buy = M('Show_buy')->where($where)->find();
					}//自己的课，不用购买就能看
				}
		...
		if($kind == 2){
			...
			$time = time();
			foreach ($c as $k => $v){	
				$e = M('Yuyue') ->where("y_part = '$v[yid]'") -> select();
				// dump($e);
				// die;
				foreach ($e as $kk => $vv) {
					$f = M('Room')->where("room_id = '$vv[y_roomid]'")->find();
					// dump($f);
					// die;
					$time1 = strtotime($f['s_time']) + 7200; 
					if($f['status'] == '1'){
						$b[$k]['room_id'] = $f['room_id'];
						$b[$k]['status'] = '1';
						$b[$k]['total_money'] = $f['total_money'];
						break;
					}
					if ($f['status'] == '0') {
						if($time > $time1){
				...
				foreach ($e as $key => $val) {
					$h = M('Room')->where("room_id = '$val[y_roomid]'")->find();
					if($h['status'] == '0'){
						$b[$k]['s_time'] = $h['s_time'];
						break;
					}
				...
				// dump($f);
				// die;
				// $d = M('Room')->where("room_id = '$v[y_roomid]' and status = '1'")->find();
				// dump($d);exit;
				// if($d){
				...
				// }else{
				// 	$ly = M('Room')->where("part = '$v[yid]'")->select();
				// 	$faa = 0;
				// 	foreach ($ly as $key => $value) {
				// 		$s_time = strtotime($value['s_time'])+7200;
				// 		if($s_time-time()>0){
				// 			$faa = 1;
				// 		}
				// 	}
...
//由琴行进入直播列表
	public function room_list1(){
		...
		// dump($a);
		// die;		         
		$c= M('Room')->where("pinbi = '0' and jxid = '$id' and status = '1'  and 
		  ...
		  $time = time();  
		 foreach($c as $k=>$v){
		 	 $active = strtotime($v['activitytime']);
		 	 $rid = $v['r_id'];
		 	if($time - $active > 60){
		 		M('Room') -> where("r_id = '$rid'") ->setField('status','2');
		 	}
		 }      
		$a= M('Room')->where("pinbi = '0' and jxid = '$id' and status = '1'  and 
			                  r_zbid = '$id' and zb_kind = '1'")
		             ->join('piano_zhubo_reg on piano_zhubo_reg.zb_id = piano_room.zhubo_id')
		             ->join('piano_dealers on piano_dealers.jxid = piano_zhubo_reg.r_zbid')
		             ->limit(($page-1)*10,10)
	...
	public function peixun_list(){
		...
		foreach ($a as $k => $v){
			$yuyue = M('yuyue') -> where("y_roomid = '$v[room_id]'") -> find();
			if($yuyue['y_status'] != 2){
				$a[$k]['price'] = $yuyue['y_shoufei'];
			}else{
				$a[$k]['price'] = $yuyue['y_video_shoufei'];
			}
	...
	public function room_list_sou(){
		...
		$type = I('type')?I('type'):0;
	 	...
	 	if($type == 0){
			$a = M('Room')->where("pinbi = '0' and (title_name like '%$key%' or 
			                       room_id like '%$key%' or zhubo_name like '%$key%') 
			                       and kind = '0' and status != '2'")
			              ->join('piano_users on piano_users.id = piano_room.zhubo_id')
			              ->limit(($page-1)*10,10)
			              ->order('s_time')
			              ->select();
			...
			foreach($a as $k=>$v){
	 		...
	 		// 	$b[$k]['url'] = $c['url'];
	 			...
	 			if($v['kind'] == 0){
	 				if($userid){
						$where = "show_userid = '$userid' and show_room_id = '$v[room_id]'";
		...
		if($type == 1){
			$b = M('Dealers')->where("jx_zhiboid = '3' and jx_name like '%$key%'")
			                 ->limit(($page-1)*10,10)
			                 ->order('jxid')
			                 ->field('jx_name as title_name,jxid as kid,jx_img as img')
			                 ->select();  
		...
		if($type == 2){
			$a = M('Yuyue')->where("y_status = '1' and y_title_name like '%$key%'
			                        and y_part = '0' and kind = '2'")
			               ->select();
	...
	public  function check_user_token(){
		...
		$a = M('Token')->where("t_userid = '$id'")->getField('t_token');
		if($token == $a){
			$result['status'] = true;	 
		}else{
			$result['status'] = false;
...
//提交订单
    public function add_goods(){
    	...
    	$data['go_time'] = date('Y-m-d H:i:s',time());
...
//支付页面
    public function show_pay(){
		...
		if($pay_mode == 3){
			...
			if(bccomp($point,$a['go_money'],1) == '-1'){
				//乐币不够的话,表示前台判断错误，只需返回失败即可
				$this->error2('乐币不够',2,'乐币不够');	
			}else{
				$new_num = bcsub($point,$a['go_money'],1);
				M('Users')->where("id = '$stuid'")->setField('money',$new_num);

				$yu_money = bcsub($a['go_money'],bcdiv($point,10,2),2);

				//混合支付，需要更新订单里的现金数和乐币数
				$data = array('go_lebi'=>$point,'go_money'=>$yu_money,'go_status'=>3);
				M('Goods_order')->where("go_num = '$num'")->save($data);
			
				if($type ==1){//支付宝
    				$result['orderSn'] = $a['go_num'];
		    	}elseif($type == 2){//微信
		    		$result['Wxpay'] = $this->careat_Wxpay_sn($num,$yu_money,'支付');
		    	}
		...
		if($pay_mode == 1){
			if($type ==1){//支付宝
    			...
    			$result['Wxpay'] = $this->careat_Wxpay_sn($num,$a['go_money'],'支付');
...
//成交
    public function sure_order(){
    	...
    	$status = $a['go_status'];
    	if($status == 1){
    		$b = M('Goods_order')->where("go_num = '$num'")->setField('go_status','2'); 
    	}
		$stuid = $a['go_userid'];
...
//提交作业
    public function homework_up1(){
		...
		if($total_homework == $total_num){
			...
			$user_stu = M("Users")->where("id='{$userid}'")->find();
			//$content = $teacher['t_truename']."老师,您的学生：".$user_stu['nickname'].' 提交了作业，请批改';
			$content = "您的学生：".$user_stu['nickname'].' 提交了作业，请批改';
...
//id老师布置的作业ID，学生ID,曲子ID，曲子名称					
			...
			if($stu_loing){
				if($stu_loing['t_leixin'] == 2){//新版本后，IOS消息推送升到V3
					...
					$this->course_add_tuisong($content,$stu_loing['t_shebei'],$stu_loing['t_leixin'],$a['t_id'],'批改作业提醒',5);
				}
			}
		}
...
//下课
	public function course_end(){
		...
		$kid = I('kid');
		$a = M('Course_list')->where("id='{$kid}'")->find();
		if($a['cl_tid'] != $id){
			$this->error2('该用户不是带班老师',2,'不是带班老师');
		}
		if($a['cl_etime'] > time()){
			$this->error2('时间未到,不能下课',2,'时间未到');
		}
...
//我的班级
	public function show_my_class(){
		...
		$key = I('key');
		$a = M('Users')->where("id = '$id'")->getField('role'); //查询这个用户的身份
		...
		if($a == 1){
			...
			$b['teacher'] = M('Teacher')->where("t_userid = '$b[class_teacher]'")
			 ...
			 //查询班级老师真实姓名              
			if($b['class_stuids'] != '' ||$b['class_stuids'] != null){ //如果该班有学生

				$c = explode(',',$b['class_stuids']); //把该班学生分成一个数组
				if($key){
				...
				foreach ($c as $k => $v){
					...
					$long = M('Practice_log')->where("log_userid = '$v' and UNIX_TIMESTAMP(log_time)
													BETWEEN '$time1' and '$time2'
		")
					                         ->sum('log_long');

					$d[$k]['long_time'] = round($long/60,1);
			...
			array_multisort($ages, SORT_DESC, $d);
...
//添加学生
	public function add_students(){
		...
		foreach ($stuid as $k => $v){
			...
			if(M('Users')->where("id = '$v'")->getField('role') == 2){
				$this->error2('不能添加其他老师为学生',2,'');
			}
		}
...
//删除学生	 
	...
	public function search_user(){
		...
		$b = M('Users')->where("username like '%$phone%' and role != '2'")
		...
		foreach ($b as $k => $v){
			...
			if(stripos($other_stu,$v['userid']) !== false){
				$b[$k]['is_student'] = 3;  //在其他班级
			}
		}
...
//我的直播  
	public function show_my_live(){
		$id = I('userid');
		$kind = (int)I('kind')?(int)I('kind'):0;
		$page = I('page')?I('page'):1;
		$part = I('part');
		$status = I('status')?I('status'):0;    //0表示预约  1表示直播

		 file_put_contents("/web/project/lelego.bofanchina.com/2.txt", $kind);
	if($status == 0){ //预约情况
		if(!$kind || $kind == 0){
		...
		if($kind == 2){
			$a = M('Yuyue')->where("kind = '2' and y_part = '0' and zhubo_id = '$id'")
			               ...
			               ->select();
			 file_put_contents("/web/project/lelego.bofanchina.com/1.txt", json_encode($a));
			foreach($a as $k => $v){
				$b = $v['yid'];
				...
				if($a[$k]['total_num'] == 0 || $a[$k]['finish_num'] == 0){
					$a[$k]['percent'] = 0;
				}else{
...
//添加作业  5-20 新版需求
	public function homework_add1(){
		...
		foreach($stu_arr as $v){
			...
			//给登陆学生推送消息
			$stu_loing = M("Token")->where(array('t_userid'=>$v))->find();
			//echo M("Token")->getLastSql();exit;
			//var_dump($stu_loing);exit;
			if($stu_loing){
				$dtime = time()+10;
				// dump($dtime);
				// die;
				...
				if($stu_loing['t_leixin'] == 2){
					$response = $client->push()
								    ->setPlatform('ios')
								    ->addAlias($stu_loing['t_shebei'])
								    ->iosNotification('老师给你布置了一个作业,请留意,记得完成哦~', [
													  'sound' => 'sound.caf',
													  'badge' => '1',
													  'extras' => [
													    'key' => 'value'
				 ...
				 //    ->setPlatform('ios')
				 //    ->addAlias($stu_loing['t_shebei'])
				 //    ->setNotificationAlert('老师给你布置了一个作业,请留意,记得完成哦~')
				 //    ->send();	

// $content1 = "老师给你布置了一个作业,请留意,记得完成哦~"
// 					$this->ios_msg_send($content,$stu_loing['t_shebei']);
					// $payload = $client->push()

					// 							  ->setPlatform('ios')	               
					// 							 ->addAlias($stu_loing['t_shebei'])
					// 							 ->setNotificationAlert("老师给你布置了一个作业,请留意,记得完成哦~", array(
					// 					              'sound' => 'sound.caf'
					//                                 ))
					// 							  ->options(array(
     //    											  "apns_production" => true  
...
//获取课程的老师点评
	public function get_course_pin(){
		$id = I('kid');
		$a = M('Course_list')->where("id = '$id' and cl_status = '2'")->field('cl_xin,cl_pin')->find();
...
//对应类别的教材
	...
	//反馈标题接口
	public function biaoti_list(){
		// $reIP=$_SERVER["REMOTE_ADDR"]; 
		// echo $reIP;exit;
		$list = M("Article")->getField('ar_title',true);
	...
	//反馈内容接口
	public function neirong_list(){
		// $reIP=$_SERVER["REMOTE_ADDR"]; 
		// echo $reIP;exit;
		$ar_id = I('ar_id');
		$list = M("Article")->where("ar_id='{$ar_id}'")->getField('ar_text');
		if($list){
			$rest['status'] = true;
			$rest['msg'] = '获取成功';
			$rest['data'] = $list;
	 ...
	 public function logResult($word='',$up='',$arr='',$user_num=null)
    {
...
//        $url = $url_buy;
    ...
    {
        $receipt_data = $_POST['receipt'];

//验证参数
        if (strlen($receipt_data)<20){
            $rest = array('buy'=>'0','message'=>'非法参数');
...
//如果是沙盒数据 则验证沙盒模式
        ...
        if($data['status']==0){
            $rest = array('buy'=>'1','message'=>'购买成功');

			 $num = (string)$_POST['order_num'];
             $cz = M('xnb_record');
             $save['xf_status'] = '2';
             $cz->where("xf_num='{$num}'")->save($save);
             $cz_user = $cz->where("xf_num='{$num}'")->field('xf_userid,xf_point')->find();
             $user = M('users');
             $user->where("id={$cz_user['xf_userid']}")->setInc('money',$cz_user['xf_point']);

        }else{
            $rest = array('buy'=>'0','message'=>'购买失败','status'=>$data['status']);
        }

//是否是沙盒模式
        if ($data['sandbox']) $rest['sandbox'] = '1';

        echo json_encode($rest);

        $this->logResult('我的苹果回调','',json_encode($rest),'');
