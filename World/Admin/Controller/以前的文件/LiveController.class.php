...
/////////////////////////////////直播室页面/////////////////////////////////////////////////////////	
	...
	public function show_jiaoxue_list(){
		$status =I('status')?I('status'):1;//状态 0：预约 ；1是直播；2是已经结束
				//主播表          直播室表
		$join1 = 'piano_users on piano_users.id = piano_room.zhubo_id';
		         //								主播id         用户id
	...
	public function show_yuyue_xiang(){
		$id = I('id');
		$join1 = 'piano_users on piano_users.id = piano_yuyue.zhubo_id';
		$list = M('yuyue') -> where("yid = '$id'") 
							->join($join1)
							-> find();

		$userid = $list['zhubo_id'];
		$yid = $list['yid'];
		$list['name'] =  M('Teacher')->where("t_userid = '$userid'")->getField('t_truename');
		$list['count'] = M('yuyue') -> where("y_part = '$yid'") -> count();
		//查出主课下所有的子课
		
		$yuyue = M('yuyue') -> where("y_part = '$yid'") -> select();
	...
	public function show_rooms_xiang(){
		...
		// e_time s_time
		if($list['status'] == 2){
				$time = strtotime($list['e_time'])-strtotime($list['s_time']);
				
				if($time < 0){
					$list['long'] = 0;
				}else{
					$list['long'] = $this->Sec2Time($time);
				}
			}elseif($list['status'] == 1){
				$time = time()-strtotime($list['s_time']); 
	...
	public function zhibo_pinbi(){
		...
		// dump($room_id);
		// die;
		$a = M('Room')->where("r_id = '$rid'")->order('r_id DESC')->find();
		// dump($a);
		// die;
		if(!$a){
			echo '查无数据';exit;
...
///////////////////////////////////预约列表页面////////////////////////////////////////////////////
	...
	public function show_yuyue_list(){
        ...
        $this->assign('status',$status);
        if($status == 1){
            $where = "y_status = '0' and y_part = '0'";
        }elseif($status == 2){
            $where = "y_status = '1' and y_part = '0'";
        }else{
            $where = "y_status = '2' and y_part = '0'";
        }
		// $join1 = 'piano_users on piano_users.id = piano_yuyue.zhubo_id';
		$join1 = 'piano_users on piano_users.id = piano_yuyue.zhubo_id';
	...
	public function yuyue_shenhe(){
		...
		if($sta ==1){
			...
			if($a['kind'] == 2){
				...
				foreach ($room_find as $k => $v) {
					...
					$r['room_id'] = $roomid;
					$r['zhubo_name'] = M('Zhubo_reg')->where("zb_id = '$v[zhubo_id]' and zb_kind = '2'")
			...
			if($c['t_leixin'] == 2){
				...
				//$f= $this->ios_msg_send($content,$c['t_shebei'],$a['zhubo_id']);
				...
				$payload = $client->push()
												 ...
												 ->iosNotification("您申请的预约直播已审核通过", array(
										             'sound' => 'sound.caf',
										             'badge' => '+1',
		...
		if($sta ==1){
			...
			$t = strtotime($start_time);  //首场直播的开播时间
			$data['x_type'] = 7;

			$frist_room = M('Room')->where("part = '$id'")->getField('room_id');//首场直播房间号
			$data['x_kid'] = $room_id?$room_id:$frist_room;
			$data['x_info'] = json_encode(array('room_id'=>$room_id?$room_id:$frist_room,
		...
		// 										 ->addAlias($c['t_shebei'])
		// 										 // ->setNotificationAlert("老师给你布置了一个作业,请留意,记得完成哦~")
		// 										 ->iosNotification("老师给你布置了一个作业,请留意,记得完成哦~", array(
		// 								             'sound' => 'sound.caf',
		// 								             'badge' => '+1',
		// 								             'content-available' => true,
		// 								             'mutable-content' => true,
	...
	public function rz_dealers(){
		$id = I('jxid');
		$type = I('type');
		$jx = M('Dealers')->where("jxid = '$id'")->find();
		$userid = $jx['jx_userid'];
		// dump($type);
		// die;
		...
		if($a){
			if($type == 3){
				...
				'x_info' => json_encode(array('kind'=>5))
				);
	...
	public function show_jiubao_list(){
		$status = I('status')?I('status'):1;
		$this->assign('status',$status);

		$where = "jiubao = '2' and jib_status = '$status'";

		$count = M('Ub_guanxi')->where($where)->count();
		//实例化分页类
   	    $Page = new \Think\Page($count,$this->pagenum);
   	    //设置上一页与下一页
   	    $Page->setConfig('prev', '上一页');
		...
		$a = M('Ub_guanxi')->where($where)
		                 ->limit($Page->firstRow.','.$Page->listRows)
		                 ->join('piano_users on piano_users.id = piano_ub_guanxi.user_id')
		                 ->join('piano_room on piano_room.room_id = piano_ub_guanxi.room_id')
		                 ->order('jib_time DESC')
		                 ->select();
	...
	public function jiubao_list_sou(){
		$status = I('status')?I('status'):1;
		$this->assign('status',$status);
	   ...
	   if($stime){
     		$where .= " and UNIX_TIMESTAMP(jib_time) >= UNIX_TIMESTAMP('$stime 00:00:00')";
     		$this->assign('stime',$stime);
     	}
     	 
     	$etime = trim($_POST['etime']);
     	if($etime){
     		$where .= " and UNIX_TIMESTAMP(jib_time) <= UNIX_TIMESTAMP('$etime 23:59:59')";
     	...
     	if(empty($stime) && empty($etime)){
     		//如果是点击页码发起的请求，则按只保存的条件搜索
     		if(!$button){
     			if($_SESSION['jiubao_list_sou']){
     				$where = $_SESSION['jiubao_list_sou'];
     			}
     		}
     	}
     	$_SESSION['jiubao_list_sou'] = $where;
		...
		//显示分页信息
		$show = $Page->show();// 分页显示输出 ->limit($Page->firstRow.','.$Page->listRows)
		$join1 = 'piano_users on piano_users.id = piano_ub_guanxi.user_id';
   	    $field = 'piano_ub_guanxi.*,piano_users.nickname';
		$list = M('Ub_guanxi')->where($where)
							  ->join($join1)
							  ->field($field)
		                 ->limit($Page->firstRow.','.$Page->listRows)
		                 ->order('jib_time DESC')
		...
		foreach ($list as $k => $v){
			$list[$k]['zhubo_name'] = M('Room')->where("room_id = '$v[room_id]'")->getField('zhubo_name');
		}
	

		$this->assign('list',$list);
		$this->assign('page',$show);
		$this->display('show_jiubao_list');

	}


	public function shenhe_jiubao(){
		$id = I('gx_id');
		$sta = I('status');

		$a = M('Ub_guanxi')->where("gx_id = '$id'")->setField('jib_status',2);
		if($a){
			echo 1;exit;
		}else{
			echo '处理失败';exit;
