		...
		<div class="col-xs-12">
			 <div class="row" style="padding-bottom: 10px;">
							<div class="col-sm-12">
								<div id="sample-table-2_filter">
									<form class="form-horizontal" action="<?php echo U('Admin/User/user_list_sou');?>" method="POST" role="form" enctype="multipart/form-data">
			...
			<!--  <script type="text/javascript" src="/Public/Admin/admin/js/region_select.js"></script>
										<div class="btn-group" style='margin-right:12px;margin-top:5px;'>
											<span style='margin-left:12px;'>所属区域：</span>
																	
								            <span style='margin-right:12px;'>省级 </span> <select name="location_p" id="location_p" class="tongselect"></select>
								           <span style='margin-left:12px;margin-right:12px;'>市级</span><select name="location_c" id="location_c" class="tongselect"></select>
								            <span style='margin-left:12px;margin-right:12px;'>区级</span>     <select name="location_a" id="location_a" class="tongselect" ></select> 
			...
			<a href="index.php?m=Admin&c=User&a=user_daochu" class="btn btn-xs btn-success">导出文件</a>
								<!-- <div style=' width:100%; height:40px; '>
								   <a href="<?php echo U('Daili/index/student_add');?>" target="rightFrame"><span style=' width:100px; height:34px; background:#2679b5; color:#fff; margin-right:10px; display:inline-block; line-height:34px;text-align:center; borde-radius:4px;'>添加学员</span></a>
							...
							<?php foreach($list as $k=>$v){?>
								<tr>
								   <!--  <td><input type='checkbox' name='' value=''></td> -->
									...
									<td><?php if($v['rz_status']==1){echo '是';}else{echo '否';}?></td>
									<td><?php if($v['role']==1){echo '学生';}elseif($v['role']==2){echo '老师';}else{echo '普通用户';}?></td>
									...
									<td>
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
												<a href="index.php?m=Admin&c=User&a=user_xiang&id=<?php echo $v['id']?>" class="btn btn-xs btn-success">详情</a>	
											</div>
										<?php if($v['role'] ==3){?>  
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group" style="margin-left:45px;">
												<a href="index.php?m=Admin&c=User&a=show_change&id=<?php echo $v['id']?>" class="btn btn-xs btn-success">成为学生</a>	
											</div>
